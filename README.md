...
1. `d = min(leftBest, rightBest)`.
2. A **strip** of points within distance `d` of the dividing vertical line is built, ordered by y-coordinate.
3. For each point in the strip, only the next few points (y-difference `< d`) are checked — at most a constant number of candidates (7–8) ever need checking per point.
